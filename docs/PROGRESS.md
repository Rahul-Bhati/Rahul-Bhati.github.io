...
| M3 | Dashboard + analytics | M2 login works; `/admin` guarded | ▶️ ready |
| M4 | Blog CMS (+ media upload) & public blog pages | M3 dashboard renders | ⬜ blocked on M3 |
...
- **Next 16: Middleware → Proxy.** Guard lives in root `proxy.ts` (Edge, optimistic JWT check); authoritative check in `getSession()` (Server Components/Actions).
